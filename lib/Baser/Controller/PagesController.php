		...
		if (empty($this->request->data)) {
			...
			if ($this->Page->validates()) {
				if ($this->Page->save($this->request->data, false)) {
					// キャッシュを削除する
					if ($this->Page->allowedPublish($this->request->data['Page']['status'],
						$this->request->data['Page']['publish_begin'], $this->request->data['Page']['publish_end'])) {
						clearViewCache();
					}
					// 完了メッセージ
 ...
 */
	public function admin_edit($id) {
		...
		if (empty($this->request->data)) {
			...
			if ($this->Page->validates()) {
				if ($this->Page->save($this->request->data,false)) {
					...
					$beforeStatus = $this->Page->allowedPublish($before['Page']['status'], $before['Page']['publish_begin'],
																$before['Page']['publish_end']);
					$afterStatus = $this->Page->allowedPublish($this->request->data['Page']['status'],
																$this->request->data['Page']['publish_begin'], $this->request->data['Page']['publish_end']);
					if ($beforeStatus != $afterStatus || $before['Page']['title'] != $this->request->data['Page']['title'] ||
						$before['Page']['url'] != $this->request->data['Page']['url']) {
						clearViewCache();
					} else {
		...
		if (empty($this->request->data['PageCategory']['id']) || $this->request->data['PageCategory']['name'] == 'mobile' ||
			$this->request->data['PageCategory']['name'] == 'smartphone') {
		...
		if (Configure::read('BcApp.smartphone') && (!isset($this->siteConfigs['linked_pages_smartphone']) ||
			!$this->siteConfigs['linked_pages_smartphone'])) {
			$reflectSmartphone = true;
		} else {
			$reflectSmartphone = false;
		}
